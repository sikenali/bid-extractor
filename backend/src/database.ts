...
export function initializeDatabase() {
  ...
  // Seed default rules — only if no rules exist at all
  const existingRules = (db.prepare('SELECT COUNT(*) as count FROM extraction_rules').get() as any).count;
  if (existingRules > 0) {
    // Check if critical groups are missing rules and seed them
    const missingGroups = [];
    for (const group of ['info', 'business', 'tech', 'score']) {
      const cnt = (db.prepare('SELECT COUNT(*) as c FROM extraction_rules WHERE group_name=?').get(group) as any).c;
      if (cnt < 5) missingGroups.push(group);
    }
    if (missingGroups.length > 0) {
      console.log('[migration] Seeding missing rule groups:', missingGroups.join(', '));
      seedMissingGroups(missingGroups);
    }
    return;
  }
...
// ── Migration: seed missing rule groups ──────────────────────────────
function seedMissingGroups(missingGroups: string[]) {
  const insert = db.prepare(
    'INSERT OR IGNORE INTO extraction_rules (id, field_name, pattern, category, group_name) VALUES (?, ?, ?, ?, ?)'
  );

  const groupSeedRules: Record<string, Array<[string, string, string, string]>> = {
    info: [
      ['info_project_name', '项目名称',
       '(?:(?:项目|采购|工程|服务)\\s*(?:名称|内容|概述|概况)|招标内容|本次招标(?:内容|范围)|采购内容)[：:]?\\s*((?:(?!(?:项目|招标|采购)编号|招标方式|采购方式)[^\\n]){6,})',
       'regex'],
      ...
      ['info_procurement_need', '采购需求',
       '(?:采购需求|项目需求|采购标的|采购内容|项目概况)[：:]?\\s*([^。\\n]{4,200})',
       'regex'],
      ['info_keyword_sme', '面向中小微', '', 'keyword'],
      ['info_keyword_energy_save', '节能环保', '', 'keyword'],
      ['info_keyword_import', '进口产品', '', 'keyword'],
    ],
    business: [
      ['business_payment', '付款方式',
       '(?:(?:付款|支付|结算)\\s*(?:方式|条件|办法|条款)|合同价款[^\\n]{0,4}支付)[：:]?\\s*([^\\n]+)',
  ...
  for (const group of missingGroups) {
    const rules = groupSeedRules[group];
    if (!rules) continue;
    for (const [id, fieldName, pattern, category] of rules) {
      insert.run(id, fieldName, pattern, category, group);
    }
  }
}
